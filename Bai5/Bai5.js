...
window.addEventListener("scroll", function () {
    document.getElementById('weather').style.height= '220px';
    document.getElementById('weather').style.transition = '2s';
    document.getElementById('weather').style.borderRadius = '20px';
    document.getElementById('weather').style.opacity = '0.7';
});
setInterval(() => {
    chuyen()
}, 100);
var flag = false;
function chuyen() {
    var arr_loi = document.getElementsByClassName('loi');
    b =arr_loi.length;
    var tongdem = 0;
    for(let i = 0; i < arr_loi.length; i++){
        if(arr_loi[i].innerText == ''){
            tongdem+=1;
        }
    }
    if (tongdem == b) {
        document.getElementById('hienthongbao').innerText = 'Chúc mừng! Bạn đã nhập đúng.';
        flag = true;
    }else{
        document.getElementById('hienthongbao').innerText = null;
        flag = false;
    }
}
function kiemtra() {
    if(flag == true){
        hoa();
      var trong = `<div class="ketquathongbao" data-aos="fade-down" data-aos-duration="2000" id="stylethong">Thành công!
                  <button class="tatdi" onclick="tat()">x</button>
                  </div>`;
      document.getElementById('ketquathongbao').innerHTML = trong;
    }

}
function tat() {
  document.getElementById('ketquathongbao').innerHTML = '';
}
function hoa() {
    "use strict";
